...
result += [tab + "sub rsp, 144  ; 分配栈空间（保持16字节对齐：push rbp 占8字节，144+8=152，不对齐）"]
    ...
    try:
        ...
        # ============ 函数调用 ============
            ...
            if four[i][0] == "arg":
                ...
                if four[i][3] == "output_float" or (four[i][3] == "output" and last_arg_is_float):
                    # 输出浮点数
                    if pending_args:
                ...
                elif four[i][3] == "output_int" or four[i][3] == "output":
                    # 输出整数
                    if pending_args:
                        arg_val = pending_args[0]
                    ...
                    if four[i][1] != '_' and four[i][3] != '_':
                        # 有返回值的函数调用: ('call', func_name, '_', result_var)
                        func_name = four[i][1]
                        result_var = four[i][3]
                    else:
                        # 无返回值的函数调用
                        func_name = four[i][3] if four[i][3] != '_' else four[i][1]
                        result_var = None
                    ...
                    if result_var and result_var != '_':
                        ...
                        offset = 4 * data.index(result_var) + 4
        ...
        # ============ 函数定义 ============
            ...
            offset = 4 * data.index(param_name) + 4
        ...
        # ============ 内存操作（数组和指针）============
            ...
            offset = 4 * data.index(arr_name) + 4
            # 数组基址存储在栈上
            result += [tab + f"lea rax, [rbp-{offset + int(size)}]  ; 数组基址"]
            result += [tab + f"mov " + inx + str(offset) + "],rax  ; 保存数组指针"]
            var_type[arr_name] = 'ptr'
        
        elif op_type == "内存读取":
            # result = load addr
            result_var = four[i][1] if len(four[i]) > 1 else four[i][3]
            addr_var = four[i][3] if len(four[i]) > 3 else four[i][2]
            if result_var not in data:
                data[data.index(-1)] = result_var
            result_offset = 4 * data.index(result_var) + 4
            
            if addr_var in data:
                addr_offset = 4 * data.index(addr_var) + 4
            ...
            if value_var and addr_var:
                if isinstance(value_var, (int, float)):
                    result += [tab + f"mov ebx, {int(value_var)}  ; 存储值"]
                elif value_var in data:
                    val_offset = 4 * data.index(value_var) + 4
            ...
            # result = addr var
            result_var = four[i][1] if len(four[i]) > 1 else four[i][3]
            src_var = four[i][3] if len(four[i]) > 3 else four[i][2]
            ...
            if src_var in data:
                src_offset = 4 * data.index(src_var) + 4
